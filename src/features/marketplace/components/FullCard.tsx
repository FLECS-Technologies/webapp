...
export default function FullCard({ app, open, onClose }: FullCardProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-surface-raised rounded-2xl w-full max-w-lg max-h-[85vh] overflow-hidden shadow-2xl border border-border flex flex-col"
      ...
      >
        ...
        <div className="relative px-6 pt-8 pb-5 text-center shrink-0">
          <button
            onClick={onClose}
            className="absolute right-3 top-3 p-1.5 rounded-lg hover:bg-surface-hover transition cursor-pointer text-muted"
          >
            <X size={18} />
          </button>
          ...
          <div className="w-16 h-16 rounded-2xl bg-surface border border-border flex items-center justify-center overflow-hidden mx-auto mb-4">
            {app.avatar ? (
              ...
              <span className="text-2xl font-bold text-muted">{app.title?.charAt(0)}</span>
            )}
          </div>
          ...
          <div className="flex items-center justify-center gap-3 text-xs">
            ...
            <span className="flex items-center gap-1 text-muted">
              <Star
                ...
                color={rating > 0 ? '#F59E0B' : 'currentColor'}
              />
          ...
          {app.categories && app.categories.length > 0 && (
            <div className="flex flex-wrap justify-center gap-1.5 mt-3">
              {app.categories.map((c: ProductCategory) => (
                <span
                  key={c.id}
                  className="px-2 py-0.5 rounded-full text-[11px] text-muted border border-border"
                >
                  {c.name}
                </span>
              ))}
        ...
        <div className="px-6 py-4 border-y border-border bg-surface shrink-0">
          ...
          {installed && (
            <div className="flex flex-wrap gap-2">
              ...
              {selectedNotInstalled && (
                <UpdateButton app={app} to={selectedVersion} showSelectedVersion />
              )}
              <UninstallButton
                app={app}
                selectedVersion={selectedVersion}
                variant="button"
                onUninstallComplete={(success: boolean, msg: string, err?: string) =>
                  success ? toast.success(msg) : toast.error(msg, { description: err })
                }
              />
            </div>
          )}
          ...
          {!installable && app.requirement && (
            <div className="flex items-center gap-2 mt-3 px-3 py-2 rounded-lg bg-error/10 text-error text-xs">
              ...
              <span>
                Not compatible with {systemInfo?.arch}. Requires {app.requirement.join(' or ')}.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-5">
            <h3 className="text-[11px] uppercase tracking-wider text-muted font-semibold mb-3">
              About
            </h3>
            ...
            <div className="px-6 pb-5 border-t border-border pt-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {app.requirement.map((req: string) => {
                  const ok =
                    systemInfo?.arch && req.toLowerCase().includes(systemInfo.arch.toLowerCase());
                  return (
                    <span
                      key={req}
                      className={`px-2.5 py-1 rounded-full text-xs font-medium ${ok ? 'bg-success/15 text-success' : 'border border-border text-muted'}`}
                    >
                      {req}
                    </span>
                  );
